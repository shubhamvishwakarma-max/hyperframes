...
Voice: Kokoro-82M (local, offline), a 50/50 blend of the two Hindi male
voices (hm_omega + hm_psi) speaking English with en-us phonemes -> a mature
Indian-English male read. Speed 0.95 (~130-135 wpm measured).
...
def main():
    ...
    for item in SCRIPT:
        ...
        print(f"{lid:12s} {t:7.2f}s  +{dur:5.2f}s  asr: {' '.join(w for w, _ in words)}")
        t += dur
    ...
    timing = {"total": round(total, 3), "scenes": scenes, "lines": lines}
